...
export default function ScienceSection() {
  return (
    <section id="science" className="py-24 md:py-32 bg-gradient-to-b from-cream to-background">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
          <h2 className="text-xs md:text-sm font-bold tracking-[0.2em] uppercase text-gold-dark mb-4">The Research</h2>
          <h3 className="text-4xl md:text-5xl font-bold text-forest-deep mb-6">
            Backed by <span className="bg-gradient-to-r from-forest to-forest-deep bg-clip-text text-transparent">Real Science</span>
          </h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
          {stats.map((item, idx) => {
            ...
            return (
              <motion.div key={idx} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.08 }} className="bg-card border border-border rounded-2xl p-6 hover:shadow-md transition-shadow text-center">
                <Icon className="w-6 h-6 text-forest mx-auto mb-3" />
                <div className="text-3xl md:text-4xl font-bold text-forest-deep mb-1">{item.stat}</div>
                <div className="text-xs text-muted-foreground mb-2">{item.detail}</div>
                <div className="text-sm font-semibold text-gold-dark">{item.label}</div>
              </motion.div>
            );
          })}
        </div>
